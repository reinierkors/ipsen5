...
public class LocationRepository extends RepositoryMaria<Location>{
	...
	protected ColumnData[] getColumns(){
		return new ColumnData[]{new ColumnData<>("id", Types.INTEGER, Location::getId, Location::setId, true), new ColumnData<>("code", Types.VARCHAR, Location::getCode, Location::setCode), new ColumnData<>("description", Types.VARCHAR, Location::getDescription, Location::setDescription), new ColumnData<>("x_coor", Types.INTEGER, Location::getxCoord, Location::setxCoord), new ColumnData<>("y_coor", Types.INTEGER, Location::getyCoord, Location::setyCoord), new ColumnData<>("waterschap_id", Types.INTEGER, Location::getWaterschapId, Location::setWaterschapId), new ColumnData<>("watertype_id", Types.INTEGER, Location::getWatertypeId, Location::setWatertypeId), new ColumnData<>("watertype_krw_id", Types.INTEGER, Location::getWatertypeKrwId, Location::setWatertypeKrwId)};
	}
	...
	public List<Location> getByFilters(Integer watertypeId, Integer waterschapId, String date){
		try{
			...
			String getLocationsStr = "SELECT * FROM location JOIN sample ON " + "location.id = sample.location_id WHERE 1 = 1 ";
			if(waterschapId != 0){
