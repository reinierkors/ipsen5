...
public class UserRepository extends RepositoryMaria<User>{
	...
	private PreparedStatement psFindByEmail() throws SQLException{
		return connection.prepareStatement(findByEmailQuery);
	}
	
	private PreparedStatement psFindBySession() throws SQLException{
		return connection.prepareStatement(findBySessionQuery);
	}
	
	private PreparedStatement psSaveSession() throws SQLException{
		return connection.prepareStatement(saveSessionQuery);
	}
	
	private PreparedStatement psDeleteSession() throws SQLException{
		return connection.prepareStatement(deleteSessionQuery);
	}
	
	private PreparedStatement psEditPassword() throws SQLException{
		return connection.prepareStatement(editPasswordQuery);
	}
	
	private PreparedStatement psEditUser() throws SQLException{
		return connection.prepareStatement(editUserQuery);
	}
	...
	protected ColumnData[] getColumns(){
		return new ColumnData[]{new ColumnData<>("id", Types.INTEGER, User::getId, User::setId, true), new ColumnData<>("email", Types.VARCHAR, User::getEmail, User::setEmail), new ColumnData<>("password", Types.VARCHAR, User::getPassword, User::setPassword), new ColumnData<>("name", Types.VARCHAR, User::getName, User::setName), new ColumnData<>("group_id", Types.INTEGER, User::getGroup_id, User::setGroup_id), new ColumnData<>("waterschap_id", Types.INTEGER, User::getWaterschap_id, User::setWaterschap_id), new ColumnData<>("session_token", Types.CHAR, User::getSessionToken, User::setSessionToken), new ColumnData<>("expiration_date", Types.TIMESTAMP, User::getExpirationDate, User::setExpirationDate)};
	}
